...
#[proc_macro_derive(MessageRepliable, attributes(message_reply))]
pub fn derive_message_repliable(input: TokenStream) -> TokenStream {
    let ast = parse_macro_input!(input as DeriveInput);
    let crate_ident = crate_ident();

    net::message::derive_message_repliable(&crate_ident, ast)
        .unwrap_or_else(Error::into_compile_error)
        .into()
}
